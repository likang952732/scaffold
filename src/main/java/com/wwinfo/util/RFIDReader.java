...
public class RFIDReader {
	...
	private static RfidrecordService db = new RfidrecordServiceImpl();
	...
	private void readPassive(HashMap reader,int antennaNum){
		...
		try{
			if (devReader.InitReader_Notype(ip, antennaNum) != READER_ERR.MT_OK_ERR) {
	        	log.error("RFID InitReader failed：" + ip);
				return;
			}
		}catch(Exception e){
			e.printStackTrace();
		...
		*/
	    ...
	    for (int i = 1; i <= 4; i++) {
			...
			if (err != READER_ERR.MT_OK_ERR){
	        	log.error("RFID ParamGet for andid="+Integer.toString(i)+" failed：" + err.toString()+"("+ip+")");
		...
		if (antCount < 1){
        	log.error("RFID ParamGet antCount is zero!");
			return;
		}
		...
		if (err != READER_ERR.MT_OK_ERR){
        	log.error("RFID ParamSet failed：" + err.toString()+"("+ip+")");
			return;
		}
		*/
		/*
		...
		*/
		...
		if (err != READER_ERR.MT_OK_ERR){
        	log.error("RFID TagInventory failed：" + err.toString()+"("+ip+")");
			return;
		}
		log.debug("RFID ip: "+ip+";天线数："+Integer.toString(antCount)
   ...
   /*
    ...
    public static void settleReadCard(String cardNo, boolean isLowerPower,HashMap readerInfo){
        ...
        List<Rfidrecord> listTemp = db.getByrFidNo(cardNo);
	...
	public void analyseTimer(String name){
		...
		//List<HashMap<String,Object>> listRecord = db.getTableData("rfidrecord", condition, "rfidNo,timeAdd desc");
		List<HashMap<String,Object>> listRecord = null;
		// 现在改成读资产
	...
	private void analyseRFIDRecord(String rfidNo,
		...
		if (null != asset && !asset.isEmpty()){
			...
			if (null != readerSet && !readerSet.isEmpty()){
				...
				hmData.put("isAbnormal",curStatus == preStatus?1:0);
				hmData.put("reasonType",curStatus == preStatus?2:0);
	...
	public void matchRFIDMan(String name){
		...
		List<HashMap<String,Object>> assetRecord = null;
		...
		for(HashMap<String,Object> item : assetRecord){
			...
			List<HashMap<String,Object>> staffRecord = null;
	...
	public void checkOutDays(String name){
		...
		List<HashMap<String,Object>> listCfg = null;
		...
		List<HashMap<String,Object>> assetList = null;
