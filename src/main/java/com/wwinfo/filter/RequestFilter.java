package com.wwinfo.filter;

import cn.hutool.json.JSONObject;
import com.wwinfo.util.CheckSecritUtil;
...
public class RequestFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
       ...
       try {
            ...
            Map<String, Object> checkSecritMap = checkSecritUtil.checkSecrit(request, requestParam);
            if(checkSecritMap != null){
                JSONObject result = new JSONObject();
                result.put("status",0);
                result.put("message","success");
                result.put("data",checkSecritMap);
                outputStream(servletResponse,result.toString());
                return;
            }
        } catch (Exception e) {
            e.printStackTrace();
     ...
     */
    private void outputStream(ServletResponse servletResponse, String result){
        try{
            //String string = JSONUtil.toJsonStr(JSONResponse.failure(5002, message));
    ...
    private Map generateSignStr(JSONObject base) {
        String timestamp = base.getStr("timestamp");
        String nonce = base.getStr("nonce");
        String userId = base.getStr("userId");
        Map map = new HashMap();
        map.put("nonce", nonce);
